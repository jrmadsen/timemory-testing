...
do
    ...
    srun -n 1 -N 1 -q interactive -t ${TIME} -C ${TARGET} python ./pyctest-runner.py -j ${JOBS} ${ARG_PYTHON} ${COMMON_ARGS}  -- ${CTEST_ARGS} -- ${CMAKE_ARGS} -DCMAKE_CXX_EXTENSIONS=ON
done
...
do
    ...
    if [ -z "${CC}" ]; then continue; fi
    if [ -z "${CXX}" ]; then continue; fi
    srun -n 1 -N 1 -q interactive -t ${TIME} -C ${TARGET} python ./pyctest-runner.py -j ${JOBS} ${ARG_PYTHON} ${COMMON_ARGS} -- ${CTEST_ARGS} -- ${CMAKE_ARGS}
done
